...
pub fn build(
    ...
    let sections = json!([
        ...
        {"kind":"skill_instructions","trust":"trusted_package","content":skill_instructions},
    ...
    Ok(
        json!({"sections":sections,"sha256":format!("{:x}",Sha256::digest(serialized.as_bytes())),"size_bytes":serialized.len()}),
    )
}
...
mod tests {
    ...
    fn fails_closed_on_budget() {
        assert_eq!(
            build("a", "b", &json!({"x":"long"}), &json!([]), 4).unwrap_err(),
