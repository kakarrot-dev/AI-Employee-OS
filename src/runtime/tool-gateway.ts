...
export type ToolRunner = (tool: ToolVersion, parameters: Record<string, unknown>, context: { actionId: string; signal: AbortSignal }) => Promise<Record<string, unknown>>
...
export class ToolGateway {
  ...
  private async execute(actionId: string): Promise<ToolAction> {
    ...
    try {
      const result = await this.runner(tool, structuredClone(action.parameters), { actionId: action.id, signal: controller.signal })
  ...
  private validateParameters(tool: ToolVersion, parameters: Record<string, unknown>, sources: ToolAction['parameterSources']): void {
    ...
    for (const source of Object.values(sources)) if (!source || !['task_input', 'trusted_runtime', 'untrusted_external_content'].includes(source.kind) || typeof source.sourceRef !== 'string' || source.sourceRef.length < 1 || source.sourceRef.length > 512) throw new Error('invalid_parameter_source')
    if (tool.id.startsWith('github.repositories.search')) {
  ...
  private blockReason(action: ToolAction): string | undefined {
    ...
    if (text.some((value) => sensitivePatterns.some((pattern) => pattern.test(value)))) return 'sensitive_egress_blocked'
    for (const [key, source] of Object.entries(action.parameterSources)) {
      if (source.kind === 'untrusted_external_content' && injectionPatterns.some((pattern) => pattern.test(String(action.parameters[key] ?? '')))) return 'prompt_injection_blocked'
    }
    return undefined
  }
