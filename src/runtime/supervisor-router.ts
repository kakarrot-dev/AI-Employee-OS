...
export class SupervisorRouter {
  ...
  async createRequest(input: RouteInput): Promise<ProviderRequest> {
    ...
    return {
      ...
      input: `${instructions}\n\n[总管资料]\n名称：${configuration.name}\nSystem Prompt：${configuration.systemPrompt}${memoryContext}\n\n[Runtime Context]\n${JSON.stringify({ availableEmployees: catalog, existingMatters, authorizedDirectories: input.directories, customerAttachments: input.attachments.map(({ id, name, mediaType, size, sha256 }) => ({ id, name, mediaType, size, sha256 })), attachmentContentEvidence, recentHistory, currentMessage: input.text })}`,
  ...
  private validateDecision(value: unknown): SupervisorRouteDecision {
    ...
    if (!['direct_answer', 'create_task', 'change_task', 'ask_user'].includes(String(decision.mode))) throw new Error('invalid_supervisor_route')
    ...
    if (decision.mode === 'create_task' || decision.mode === 'change_task') {
      ...
      if (decision.mode === 'change_task' && !targetTaskId) throw new Error('invalid_supervisor_route')
    } else if (decision.employeeVersionIds.length > 0) throw new Error('invalid_supervisor_route')
    return { ...decision, targetTaskId, title: decision.mode === 'create_task' || decision.mode === 'change_task' ? normalizeMatterTitle(title, decision.goal) : '' } as SupervisorRouteDecision
  }
}
