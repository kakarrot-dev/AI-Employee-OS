...
export function createToolRunner(imageTextExtractorPath: string, feishuRunner?: ToolRunner, teamsRunner?: ToolRunner): ToolRunner {
  ...
  return async (tool, parameters, context) => {
    ...
    if (tool.id.startsWith('teams.')) {
      if (!teamsRunner) throw new Error('teams_runner_unavailable')
      return teamsRunner(tool, parameters, context)
    }
