import { TEAMS_CAPABILITY_ID, TEAMS_TOOL_IDS } from '../shared/teams-contract'
...
const BUILT_IN_CAPABILITIES: AgentCapabilityVersion[] = [
  { schemaVersion: 1, id: TEAMS_CAPABILITY_ID, createdAt: '2026-09-08T00:00:00.000Z', name: 'Teams 找人与会议', description: '查询企业通讯录并创建带日历邀请的 Teams 会议。', version: 2, skillVersionIds: ['skill.teams-meetings.v2'], toolVersionIds: Object.values(TEAMS_TOOL_IDS), mcpVersionIds: ['mcp.teams.v2'], requiredModelIds: ['deepseek-v4-pro', 'claude-sonnet-4.6'], permissionRequirements: ['teams.contacts.search', 'teams.meetings.create', 'teams.meetings.add-attendees'], dependencies: [{ kind: 'Skill', versionId: 'skill.teams-meetings.v2', available: true }, ...Object.values(TEAMS_TOOL_IDS).map(versionId => ({ kind: 'Tool' as const, versionId, available: false, reason: '请连接 Teams' })), { kind: 'MCP', versionId: 'mcp.teams.v2', available: false, reason: '请连接 Teams' }] },
...
const SPECIALIST_EMPLOYEES: Array<{ employee: Employee; version: EmployeeVersion }> = [
  { employee: { schemaVersion: 1, id: 'employee.teams-coordinator', createdAt: '2026-09-08T00:00:00.000Z', name: 'Teams 会议专员', activeVersionId: 'employee-version.teams-coordinator.v2', disabled: false, archived: false }, version: { schemaVersion: 1, id: 'employee-version.teams-coordinator.v2', createdAt: '2026-09-08T00:00:00.000Z', employeeId: 'employee.teams-coordinator', version: 2, state: 'active', name: 'Teams 会议专员', role: 'Microsoft Teams 企业联系人查询与日历会议安排', description: '按姓名找人，确认主题、时间、组织者及参会人后创建 Teams 日历邀请。', systemPrompt: '你是 Teams 会议专员。遵循 Teams 找人与会议 Skill。只找人时只调用通讯录工具，不创建会议。姓名、时间与会议主题来自用户；重名或未找到必须澄清，不猜测身份。创建前等待确认，结果未知或已尝试不得重复创建。以真实 calendarEventId 和 meetingUrl 为会议创建证据，提交邀请不表示对方接受。', modelId: 'deepseek-v4-pro', capabilityVersionIds: [TEAMS_CAPABILITY_ID], memoryScopes: ['task'], testRunIds: [], publishedAt: '2026-09-08T00:00:00.000Z' } },
