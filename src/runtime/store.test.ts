...
describe('RuntimeStore', () => {
  ...
  it('keeps immutable snapshots and audit events append-only', () => {
    ...
    const kernel = new RuntimeKernel(store)
    const entity = { schemaVersion: RUNTIME_SCHEMA_VERSION, id: 'revision-1', createdAt: '2026-08-31T00:00:00Z', taskId: 'task-1', sourceDraftId: 'draft-1', revision: 1, frozen: true as const, attachments: [], goal: 'test', acceptanceCriteria: ['pass'], employeeVersionIds: [], capabilityVersionIds: [], modelConfigIds: [], budget: { maxInputTokens: 1, maxOutputTokens: 1, maxAmountUsdMicros: 1, maxSteps: 1 }, timeoutsMs: { firstToken: 1, request: 1, run: 1 }, authorizationMode: 'approval_required' as const, resourceScope: { directories: [], toolVersionIds: [], modelConfigIds: [], memoryScopes: [] } }
