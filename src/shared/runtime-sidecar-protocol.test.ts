...
describe('runtime sidecar protocol', () => {
  ...
  it('accepts a bounded event cursor request', () => {
    ...
    expect(parseRuntimeCommand({ schemaVersion: 1, requestId: '6', type: 'conversation.send', payload: { conversationId: 'conversation-1', messageId: 'message-1', text: '写文档', directories: ['/tmp/reports'], attachments: [] } })).toMatchObject({ type: 'conversation.send', payload: { directories: ['/tmp/reports'], attachments: [] } })
