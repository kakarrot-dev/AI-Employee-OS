...
import { ChatBubble, Trash, WarningTriangle } from 'iconoir-react'
...
export function ExpertGroupModule({ group, onGroupsChanged, onEnterConversation }: { group?: ExpertGroupView; onGroupsChanged: (groups: ExpertGroupView[]) => void; onEnterConversation?: () => void }): React.JSX.Element {
  ...
  return <>
    <DetailPage className="employee-detail-page expert-group-detail-page">{error && <p className="inline-error" role="alert">{error}</p>}{group ? <ExpertGroupProfileContent group={group} mode={onEnterConversation ? 'demo' : 'directory'} actions={<>{onEnterConversation && <Button className="button button--primary" onPress={onEnterConversation}><ChatBubble aria-hidden />进入会话</Button>}<IconButton label="解雇专家团" icon={Trash} className="profile-dismiss-button" onClick={() => setDismissOpen(true)} /></>} /> : <div className="directory-empty"><h3>还没有已招募专家团</h3><p>点击通讯录右上角的招募按钮，浏览候选专家团。</p></div>}</DetailPage>
