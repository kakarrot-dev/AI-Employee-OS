...
import { Avatar, ClientModal, DetailPage, IconButton, SettingRow, SettingsBlock, StatusLight, type ClientIcon } from './components/client-ui'
...
export function TeamModule({ selectedEmployeeId, createRequest = 0, editRequest = 0, onEmployeesChanged, onSelectEmployee, onEditEmployee }: { selectedEmployeeId?: string; createRequest?: number; editRequest?: number; onEmployeesChanged?: (employees: EmployeeSummary[]) => void; onSelectEmployee?: (employeeId: string) => void; onEditEmployee?: () => void }): React.JSX.Element {
  ...
  return <>
    <DetailPage>{error && editorMode === null && <p className="inline-error" role="alert">{error}</p>}{detail ? <>
      ...
      <section className="plain-section"><div className="content-section-title"><h3>能力摘要</h3><span>{detailCapabilities.length} 项</span></div><div className="capability-summary">{detailCapabilities.length ? detailCapabilities.map((item) => <div key={item.id}><Sparks aria-hidden width={18} height={18} /><span><strong>{item.name}</strong><small>{item.dependencies.every((dependency) => dependency.available) ? detail.status === 'pending_test' ? '已绑定，测试通过后可用于正式任务' : '依赖正常，可用于正式任务' : '存在不可用依赖'}</small></span><NavArrowRight aria-hidden width={16} height={16} /></div>) : <p className="empty-state">尚未绑定能力。</p>}</div></section>
      <section className="plain-section"><div className="content-section-title"><h3>最近 3 项交付</h3><span>按时间倒序</span></div><div className="delivery-list">{recentDeliveries.length ? recentDeliveries.map((task) => <button type="button" key={task.id} onClick={() => setSelectedDelivery(task)}><span className="delivery-list__icon"><Check aria-hidden width={16} height={16} /></span><span><strong>{task.goal}</strong><small>{task.delivery?.artifacts.length ?? 0} 个交付物 · {task.delivery?.evidenceCount ?? 0} 条证据</small></span><span>已交付</span><NavArrowRight aria-hidden width={16} height={16} /></button>) : <p className="empty-state">新员工尚无交付记录。</p>}</div></section>
