...
export function TeamModule({ selectedEmployeeId, skills = [], providerStatus, createRequest = 0, editRequest = 0, onEmployeesChanged, onSelectEmployee, onEditEmployee }: { selectedEmployeeId?: string; skills?: ResourceCatalogView['skills']; providerStatus: ProviderStatus; createRequest?: number; editRequest?: number; onEmployeesChanged?: (employees: EmployeeSummary[]) => void; onSelectEmployee?: (employeeId: string) => void; onEditEmployee?: () => void }): React.JSX.Element {
  ...
  const createCanContinue = currentStepIssues.length === 0 && (createStep !== 2 || draft.capabilityVersionIds.length > 0)
  const revealCurrentStepIssues = (): void => {
    ...
    setError(createStep === 2 && !draft.capabilityVersionIds.length ? '请至少选择一项员工能力。' : '请先完成当前步骤的必填项。')
  }
  ...
  return <>
    ...
    <ClientModal open={editorMode === 'create'} title="新建 Agent 员工" eyebrow={<span className="quiet-meta">第 {createStep + 1} / 3 步</span>} size="large" onClose={() => setEditorMode(null)}><div className="modal-layout"><nav className="modal-navigation create-agent-navigation" aria-label="创建步骤">{([['基本资料', Group], ['提示词', EditPencil], ['模型与能力', Brain]] as const).map(([label, Icon], index) => <button type="button" key={label} className={createStep === index ? 'is-active' : ''} onClick={() => navigateCreate(index as 0 | 1 | 2)}><Icon aria-hidden width={17} height={17} /><span>{label}</span>{index < createStep && <Check aria-hidden width={15} height={15} />}</button>)}</nav><div className="modal-content modal-content--with-footer"><div className="modal-content__main">{error && <p className="inline-error" role="alert">{error}</p>}
      {createStep === 0 && <div className="form-section"><h2>基本资料</h2><div className="avatar-editor"><label className="avatar-upload"><input type="file" accept="image/png,image/jpeg,image/webp" aria-label="从本地上传新员工头像" onChange={(event) => changeAvatar(event.currentTarget.files?.[0] ?? null)} /><Avatar label={draft.name || '新员工'} initials={draft.name.trim().slice(0, 1) || '新'} color="#c5b8e3" size="large" src={draft.avatarDataUrl} /><span className="avatar-upload__affordance" aria-hidden="true"><EditPencil width={16} height={16} /></span></label><small>PNG、JPEG 或 WebP，最大 2 MB</small></div><label className="form-field"><span>员工名称</span><input value={draft.name} maxLength={EMPLOYEE_FIELD_LIMITS.name.max} aria-label="名称" aria-invalid={Boolean(issueFor('name'))} aria-describedby="create-name-feedback" onChange={(event) => updateDraft('name', event.target.value)} placeholder="例如：用户研究员" /><FieldFeedback id="create-name-feedback" value={draft.name} issue={issueFor('name')} {...EMPLOYEE_FIELD_LIMITS.name} /></label><label className="form-field"><span>员工职责</span><input value={draft.role} maxLength={EMPLOYEE_FIELD_LIMITS.role.max} aria-label="职责" aria-invalid={Boolean(issueFor('role'))} aria-describedby="create-role-feedback" onChange={(event) => updateDraft('role', event.target.value)} placeholder="例如：用户访谈与洞察分析" /><FieldFeedback id="create-role-feedback" value={draft.role} issue={issueFor('role')} {...EMPLOYEE_FIELD_LIMITS.role} /></label><label className="form-field"><span>职责说明</span><textarea rows={4} value={draft.description} maxLength={EMPLOYEE_FIELD_LIMITS.description.max} aria-label="职责说明" aria-invalid={Boolean(issueFor('description'))} aria-describedby="create-description-feedback" onChange={(event) => updateDraft('description', event.target.value)} placeholder="说明这个 Agent 负责什么，以及不负责什么。" /><FieldFeedback id="create-description-feedback" value={draft.description} issue={issueFor('description')} {...EMPLOYEE_FIELD_LIMITS.description} /></label></div>}
    ...
    <ClientModal open={editorMode === 'settings'} title={detail?.employee.name ?? 'Agent 设置'} identity={detail ? <div className="modal-identity employee-modal-identity"><Avatar label={draft.name || detail.employee.name} initials={(draft.name || detail.employee.name).slice(0, 1)} color="#b8c982" size="medium" src={employeeAvatarSrc({ employeeId: detail.employee.id, avatarDataUrl: draft.avatarDataUrl })} /><strong>{draft.name || detail.employee.name}</strong><StatusLight state={employeeStatusTone(detail.status)} label={employeeStatusLabel(detail.status)} breathing={employeeStatusBreathing(detail.status)} /></div> : undefined} headerMeta={canDelete ? <div className="modal-header__actions"><IconButton label="删除 Agent" icon={Trash} className="modal-delete-button" onClick={() => setDeleteOpen(true)} /></div> : undefined} size="large" onClose={() => setEditorMode(null)}>
      <div className="modal-layout">
        ...
        <div className="modal-content"><div className="modal-content__main">{error && <p className="inline-error" role="alert">{error}</p>}
          ...
          {settingsSection === 'model' && <div className="form-section employee-settings-form"><h2>模型与能力</h2><p>从多个候选模型中选择一个主模型，并组合多项 Agent 能力。</p><ModelCapabilitySelection draft={draft} capabilities={capabilities} providerStatus={providerStatus} modelQuery={modelQuery} capabilityQuery={capabilityQuery} onModelQuery={setModelQuery} onCapabilityQuery={setCapabilityQuery} onModel={(modelId) => updateDraft('modelId', modelId)} onCapabilities={(ids) => updateDraft('capabilityVersionIds', ids)} /><SettingsBlock title="派生边界"><SettingRow title="运行时授权" description="正式运行时仍与任务授权取交集"><ShieldCheck aria-hidden width={18} height={18} /></SettingRow><SettingRow title="测试与组队" description="由总管统一组织，不在员工设置中操作"><Group aria-hidden width={18} height={18} /></SettingRow></SettingsBlock></div>}
