...
struct ContactsDemoData {
    ...
    private static var sample: Self {
        ...
        let wukongPermissions = [
            EmployeePermissionItem(id: "wukong-read", name: "读取文件", resource: "filesystem.read", source: "本地文件", effect: "允许", confirmation: nil),
            EmployeePermissionItem(id: "wukong-write", name: "写入文件", resource: "filesystem.write", source: "本地文件", effect: "允许", confirmation: "每次确认"),
            EmployeePermissionItem(id: "wukong-network", name: "网络搜索", resource: "network.search", source: "Agent Reach 网络搜索", effect: "允许", confirmation: nil)
        ]
        let mayaPermissions = [
        ...
        let employees = [
            employee(id: "001", name: "悟空", role: "AI 产品经理", department: "产品部", identity: "# 悟空\n\n你是一名 AI 产品经理，负责把模糊目标转化为可执行、可验收的产品方案。\n\n## 工作范围\n\n- 澄清业务目标与用户问题\n- 输出结构化产品需求文档\n- 定义状态、异常流程和验收标准", soul: "# 工作原则\n\n- 坚持第一性原理\n- 区分事实、推测与未知\n- 结论必须可执行、可验证"),
        ...
        return Self(employees: employees, capabilities: [
            "001": .init(selectedSkills: [fileOperationsSkill, webSearchSkill], selectedTools: [fileTool, agentReachTool], permissions: wukongPermissions, skillCatalog: skillCatalog, toolCatalog: toolCatalog),
