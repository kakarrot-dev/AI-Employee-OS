...
- Material 只用于 Popover 和临时浮层，主工作区不使用装饰性玻璃控制岛。
- 动效表达状态，不承担装饰。
...
Sidebar 默认展开并保留 macOS 原生折叠能力。它是持续工作上下文，不是四按钮启动器，从上到下展示：

1. 当前企业身份与命令入口。
2. 四个平铺主入口。
3. 一层部门及其真实员工。
4. 最近工作，最多显示八条真实 Task。
...
- 企业区只显示当前企业，不提供企业切换。
- 图标保持单色；Selection 使用 Claude Cream Primary 的低对比底色，不使用系统蓝破坏统一色系。
- 员工与最近工作直接恢复上下文；当前 MVP 只显示真实 Alex 和真实 Task，不使用占位员工。
- 同一时刻只允许一个视觉选中项，主入口、员工和最近工作不得双重高亮。
...
- 开放式办公室，不使用封闭房间和高墙。
- MVP 默认采用 `3 列 × 2 行` 的六工位规则网格，工位共享统一尺寸和朝向。
- 工位本身是画面主体，不用地毯、绿植、矮柜或共享桌填补容量位置。
- 工位随主工作区尺寸均匀伸缩，员工始终清晰可定位。
- 使用固定镜头；不支持拖拽、旋转和缩放。
...
- 固定温暖日间自然光。
- 光源方向稳定，家具、人物和铭牌阴影一致。
- 背景使用 `Canvas`，桌面与纸张使用 `Surface Soft / Surface Card`，轮廓使用 `Hairline`。
...
- 部门名称和真实容量显示在工位网格上方，例如“产品部，1 名员工，5 个空工位”。
- 当前 MVP 不增加独立部门标牌、地面文字或整块状态染色。
...
Office 直接使用窗口原生 Toolbar，不在场景上叠加第二套悬浮控制条。

- 左侧标题显示“办公室”。
- 右侧提供办公室与列表视图切换，以及“交给 Alex 新工作”。
- 全局搜索进入 Sidebar 企业区的命令入口，不在 Office 重复搜索框。
- 创建员工只在真实能力开放后加入 Toolbar；当前 MVP 不展示不可用按钮。
- Toolbar 不显示企业统计指标，场景顶部不为装饰性控制岛预留空白。
...
- 当前只有 Alex 时仍展示六个真实容量工位，Alex 占用第一个，其余五个显示为空工位。
- 空工位不创建虚构同事、虚构任务、虚构状态或不可用的创建操作。
- 空工位只显示统一桌、椅和关闭状态的屏幕，不显示姓名铭牌。
