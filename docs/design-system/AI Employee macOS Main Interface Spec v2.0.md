...
```text
┌────────┬────────────────────┬────────────────────────────────────┐
│ 模块轨 │ 上下文 Sidebar     │ Workspace                          │
│ 60pt   │ 280pt              │ Office / Directory / Conversation │
│        │ 248...320pt        │ / Capability Library              │
└────────┴────────────────────┴────────────────────────────────────┘
```

- 模块轨固定为 `60pt`。
- 通讯录和工作模块的上下文 Sidebar 默认 `280pt`，允许在 `248...320pt` 之间调整。
- 办公室和能力库不显示上下文 Sidebar，模块轨后直接进入内容区。
- Workspace 使用剩余空间。
- 窗口达到最小宽度时，模块轨保持可见，上下文 Sidebar 可以折叠。
- Inspector 不占用固定第三列，只在当前任务有可检查内容时出现。

## 5. 模块轨

模块轨只负责一级模块切换，不显示员工、会话或任务。
...
- 使用单色 SF Symbols。
- 选中态使用 Claude Cream Primary 的低对比圆形或圆角矩形背景。
- 图标必须有 Tooltip 和辅助功能标签。
- 同一时刻只显示一个一级模块选中态。
- 不显示商城、充值、额度、移动端和付费入口。
- 设置入口打开独立 Settings Scene，不替换主窗口 Workspace。

## 6. 上下文 Sidebar

上下文 Sidebar 只服务于需要列表选择的模块。通讯录和工作保留该层；办公室和能力库不渲染 Sidebar，也不显示折叠按钮。
...
- 不显示上下文 Sidebar。
- 模块轨后直接展示工位画布、员工状态与空工位。
- 点击真实员工工位查看状态并进入对应工作上下文。
- 不重复展示员工列表或最近工作列表。
...
- 不显示上下文 Sidebar。
- 作为全局能力目录，统一罗列系统中全部 Skill 与 Tool，不绑定当前 Employee。
- 顶部可按全部、Skills、Tools 筛选。
- 每项展示名称、类型、版本、状态和适用范围；不得把执行历史误称为完整安装目录。
- Runtime 尚未提供目录数据时显示明确空状态，不使用占位卡或禁用按钮填充页面。
...
- 模块轨保持 `60pt`。
- 上下文 Sidebar 使用 `248pt` 或折叠。
- Inspector 默认收起。
- Composer 水平边距降为 `16pt`。
- Toolbar 隐藏岗位和次要按钮。
- Composer 可用宽度不得低于约 `480pt`。
