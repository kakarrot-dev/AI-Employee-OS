...
## 12. Phase 10：本机专员员工扩展

实施与验收记录：[Phase 10：网络情报员与文档编写员实施验收](audits/phase-10/specialist-employees-and-local-tools.md)。

### 工作与门禁

- 安装网络情报员和文档编写员，能力互斥并按网络调研 → 文档交付串行协作。
- 网络扩展只调用当前用户已安装的 Agent-Reach、Last 30 Days、OpenCLI 固定只读入口；不改变 Phase 9 默认分发包的零全局依赖门禁。
- 文档 Tool 只在用户通过原生目录选择器授权的 RunGrant 目录内工作，创建不覆盖、编辑唯一匹配、写后回读 Hash。
- 外部依赖、登录态或来源失败必须传播到员工能力或 ResearchBundle 信息缺口，不能静默降级。
