...
2026-09-07 用户授权扩展：[飞书会议场景](feishu-meeting-scenario.md)。新增内置会议助理、可选会议授权、组织内联系人搜索、确认创建及确认发送邀请。会议外部写入总是需要具体动作确认，此范围优先于下文通用“完全访问”规则；文档 MVP 其余边界不变。
...
- 用户可配置总管头像、名称、System Prompt、文本模型，以及是否读取已治理的全局记忆；配置由 Runtime 持久化并用于后续路由、直接回答与验收调用。
- System Prompt 只定义沟通、任务拆解和验收偏好。平台注入的权限、安全、路由 Schema 和证据规则不可被覆盖。
- 不能替代缺失员工直接完成业务工作。
...
| 对象 | 用户权限 | 来源 |
| --- | --- | --- |
| 总管定义 | 可编辑头像、名称、System Prompt、模型和全局记忆开关 | Runtime 单例配置；平台安全与权限规则只读 |
...
员工界面状态只表达“现在能否工作”：草稿、待测试、可工作、已停用、已归档。已发布员工即使另有未发布草稿，整体仍显示“可工作”；草稿的保存、测试和发布进度只在员工设置中展示。
...
客户端在等待 Agent 首个输出时，必须通过通用 `AgentActivityMessage` 组件在时间线显示“Agent 身份 + 思考中”，不使用消息气泡；收到首个输出后由同一消息组件体系切换为流式结果。瞬时活动状态不保存为 Message，历史记录不得重放该动效，并遵循系统减少动态效果设置。
...
- 员工结果使用统一 `AgentHandoffEnvelope` 传递：文本、结构化 JSON、产物引用、证据引用、可恢复错误和命名空间扩展保持原类型；Runtime 校验版本、接收方、大小与完整性 Hash 后才允许下游读取。
